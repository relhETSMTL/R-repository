...
# --------------------------------------------------------------------------------------------------
# Question 1
participant02.Q01 <- read.csv(file = "../../Experiment-Data/Eye-tracking-data-samples/PartP02/P02-TOI-Q17-Act01-Data.csv", 
                              header=TRUE,
                              fileEncoding="latin1") # new parameter for encoding
...
# Corrected version
curated02.Q01 <- participant02.Q01 %>% 
  # filter(!across(everything(), is.na)) %>% 
  filter(Eye.movement.type=="Fixation" & Sensor=="Eye Tracker") %>% 
  select(Eye.movement.type, Eye.movement.type.index, Gaze.event.duration..ms., Sensor, starts_with("AOI.hit")) %>%
  na.omit() %>% # eliminating NAs after filtering and column selection
  distinct()
...
fixations.Window.Q01 <-  totalFixations.Q01 - (fixations.Answer.Q01 + fixations.Buttons.Q01 + 
                                                 fixations.CTC.Q01 + fixations.FM.Q01 + fixations.Legend.Q01 +
                                                 fixations.Question.Q01)
perc.fixations.Window.Q01 <- fixations.Window.Q01 / totalFixations.Q01
time.Window.Q01 <- totalFixationTime.Q01 - (time.Answer.Q01 + time.Buttons.Q01 + 
                                              time.CTC.Q01 + time.FM.Q01 + time.Legend.Q01 +
                                              time.Question.Q01) 
perc.time.Window.Q01 <- time.Window.Q01/totalFixationTime.Q01


# Old version of AOI Window which was basically any hit on the background
# # AOI Window - QNN for participant question
# question01.Window <- question01 %>% filter(AOI.hit..P02.TOI.Q17.Act01.Snap...Q01.Window. =="1")
...
Q01.data <- c(ParticipantID, QNumber, totalFixations.Q01, totalFixationTime.Q01,
              ...
              fixations.Window.Q01, perc.fixations.Window.Q01, time.Window.Q01, perc.time.Window.Q01,
              fixations.CTC.Q01, perc.fixations.CTC.Q01, time.CTC.Q01, perc.time.CTC.Q01)


# NEW Testing the additions

totalFixations.Q01
fixations.Question.Q01 + fixations.Answer.Q01 + fixations.Legend.Q01 + fixations.Buttons.Q01 + fixations.FM.Q01 + fixations.Window.Q01 + fixations.CTC.Q01
perc.fixations.Question.Q01 + perc.fixations.Answer.Q01 + perc.fixations.Legend.Q01 + perc.fixations.Buttons.Q01 +  perc.fixations.FM.Q01 + perc.fixations.Window.Q01 + perc.fixations.CTC.Q01 

totalFixationTime.Q01
time.Question.Q01 + time.Answer.Q01 + time.Legend.Q01 + time.Buttons.Q01 +  time.FM.Q01 + time.Window.Q01 + time.CTC.Q01
perc.time.Question.Q01 + perc.time.Answer.Q01 + perc.time.Legend.Q01 + perc.time.Buttons.Q01 + perc.time.FM.Q01 + perc.time.Window.Q01 + perc.time.CTC.Q01

# Old version of the participant data, remove Containing and Navigating, add Window
# Q01.data <- c(ParticipantID, QNumber, totalFixations.Q01, totalFixationTime.Q01, 
#               fixations.Question.Q01, perc.fixations.Question.Q01, time.Question.Q01, perc.time.Question.Q01,
#               fixations.Answer.Q01, perc.fixations.Answer.Q01, time.Answer.Q01, perc.time.Answer.Q01,
#               fixations.Legend.Q01, perc.fixations.Legend.Q01, time.Legend.Q01, perc.time.Legend.Q01,
...
participant02.Q02 <- read.csv(file = "../../Experiment-Data/Eye-tracking-data-samples/PartP02/P02-TOI-Q06-Act02-Data.csv", 
                              header=TRUE,
                              fileEncoding="latin1")
attach (participant02.Q02)

# Filters the rows with all NAs, keeps only the fixations of Eye tracker events, and repeated elements
curated02.Q02 <- participant02.Q02 %>% 
  # filter(!across(everything(), is.na)) %>% 
  filter(Eye.movement.type=="Fixation" & Sensor=="Eye Tracker") %>% 
  select(Eye.movement.type, Eye.movement.type.index, Gaze.event.duration..ms., Sensor, starts_with("AOI.hit")) %>%
  na.omit() %>%
...
participant02.Q03 <- read.csv(file = "../../Experiment-Data/Eye-tracking-data-samples/PartP02/P02-TOI-Q22-Act03-Data.csv",
                              header=TRUE,
                              fileEncoding="latin1")
attach (participant02.Q03)

# Filters the rows with all NAs, keeps only the fixations of Eye tracker events, and repeated elements
curated02.Q03 <- participant02.Q03 %>% 
  # filter(!across(everything(), is.na)) %>% 
  filter(Eye.movement.type=="Fixation" & Sensor=="Eye Tracker") %>% 
  select(Eye.movement.type, Eye.movement.type.index, Gaze.event.duration..ms., Sensor, starts_with("AOI.hit")) %>%
  na.omit() %>%
...
participant02.Q04 <- read.csv(file = "../../Experiment-Data/Eye-tracking-data-samples/PartP02/P02-TOI-Q03-Act04-Data.csv", 
                              header=TRUE,
                              )
